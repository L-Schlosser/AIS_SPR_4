...
The app already **classifies** a scanned document into one of six types
(`invoice`, `receipt`, `doctor_note`, `care_leave`, `delivery_note`,
`master_data`). Given the document type plus the raw **OCR text**, the following returns a structured JSON of the important fields with
**German UI keys**, e.g.:
...
3. **Fine-tune** The model predicts labels, a **cross-entropy loss** compares
   them to the truth, and **backpropagation** updates the weights.
   Several epochs over ~7,000 samples (1,200 per type × 6 types) are trained.

4. **Evaluate** A held-out split reports token accuracy and entity-level
   precision/recall/F1.

**Important:** it is taught by *example*, not by hand-written rules. To support a
new field a labeled examples is added for it and retrained.
...
4. **Reassemble:** merge subword labels back to words, then join consecutive
   `B-/I-` of the same type into one entity, for each field the **single
   best** entity (highest confidence, then longest) is kept - so a value appearing
