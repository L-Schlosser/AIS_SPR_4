...
The model is very small and has very low latency between outputs.
 It is an NLP model which can be used for the summarization task. It doesn't take a lot of memory since it has roughly 149 milion parameters which makes it very quick. It also operates only on 8192 tokens which would be within the dimension threshold of these type of models
...
It can also be fine-tuned directly and trained accordingly to the data provided.Faster than traditional Bert and much quicker also.
 
### 3.2 OCR

<!-- TODO: Describe the classification architecture in plain language --> 

It can summarize the OCR output since it is an NLP model( natural language processing ) tasks. Not built in OCR.
...
|----------------|---------------------|----------------|
| Baseline   | 39.0%                   | 1.13s              |
| This model | <!-- TODO --> 7.3%     | <!-- TODO -->0.16s  |
---
...
- It is easy to load since the model is part of huggingface.
 -  It takes almost no time to be stored on the storage since it is small. 
- It has bad accuracy but good latency response for it's size.
...
**Reasoning:**
The results were very below the required threshold although the latency was within the threshold.
